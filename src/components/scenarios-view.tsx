...
import { allTasks, scenarios as mockScenarios, type Task, type Scenario } from '@/app/lib/mock-data';
...
export default function ScenariosView() {
  ...
  const [activeScenarioId, setActiveScenarioId] = useState<Scenario['id']>('A');
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);

  const activeScenario = scenarios.find(p => p.id === activeScenarioId)!;
    ...
    setScenarios(prevScenarios =>
      prevScenarios.map(scenario => {
        if (scenario.id === activeScenarioId) {
          ...
          if (to === 'included') {
            newScenario.excludedTasks = newScenario.excludedTasks.filter(id => id !== taskId);
            newScenario.includedTasks.push(taskId);
          } else {
            newScenario.includedTasks = newScenario.includedTasks.filter(id => id !== taskId);
            newScenario.excludedTasks.push(taskId);
          }
          return newScenario;
        }
        return scenario;
  ...
  const includedTasks = allTasks.filter(t => activeScenario.includedTasks.includes(t.id));
  const excludedTasks = allTasks.filter(t => activeScenario.excludedTasks.includes(t.id));
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-4">
          <Select value={activeScenarioId} onValueChange={(val: Scenario['id']) => setActiveScenarioId(val)}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="lg:col-span-4">
          <CardHeader>
            <CardTitle>Included in Scenario</CardTitle>
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <CardTitle>Scenario Summary & Exclusions</CardTitle>
